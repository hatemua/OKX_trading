-- Supabase Database Schema for OKX Trading Bot
-- This schema stores all trades with strategy information for analytics
...
CREATE INDEX idx_trades_subcategory ON trades(subcategory);
CREATE INDEX idx_trades_strategy_ref ON trades(strategy_ref);
CREATE INDEX idx_trades_executed_at ON trades(executed_at);
CREATE INDEX idx_trades_created_at ON trades(created_at);
CREATE INDEX idx_trades_status ON trades(status);

-- Composite indexes for common queries
CREATE INDEX idx_trades_coin_subcategory ON trades(coin, subcategory);
CREATE INDEX idx_trades_strategy_status ON trades(strategy_ref, status);
CREATE INDEX idx_trades_coin_action_date ON trades(coin, action, executed_at);
...
CREATE INDEX idx_strategy_performance_coin ON strategy_performance(coin);
CREATE INDEX idx_strategy_performance_subcategory ON strategy_performance(subcategory);
CREATE INDEX idx_strategy_performance_profit ON strategy_performance(total_profit_loss);
CREATE INDEX idx_strategy_performance_win_rate ON strategy_performance(win_rate);
...
BEGIN
    ...
    UPDATE strategy_performance SET
        win_rate = (
            SELECT (COUNT(*) FILTER (WHERE profit_loss > 0) * 100.0 / NULLIF(COUNT(*) FILTER (WHERE action = 'sell'), 0))
            FROM trades 
